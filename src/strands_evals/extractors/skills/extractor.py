...
calls are skill loads, pair each with its result, recover the body, and fold duplicate reads of
one skill together. The public entry points are `parse_available_skills` and
        ...
        for span in trace.spans:
            ...
            if skill_name is not None:
                if failed or _load_refused(span.tool_result.content):
                    ...
                    out.append(InvokedSkill(name=skill_name, body=None, status="failed", error=error))
                else:
    ...
    for message_index, _, block in indexed_blocks:
        ...
        if skill_name is not None:
            if matched_result is not None and matched_result.refused:
                out.append(InvokedSkill(skill_name, None, status="failed", error=matched_result.error))
                continue
        ...
        if read_path and matched_result is not None and not matched_result.refused and matched_result.body:
            out.append(InvokedSkill(_skill_name_from_body(matched_result.body, read_path), matched_result.body))
    return _deduplicate_invocations(out)
